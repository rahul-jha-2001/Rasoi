...
#         try:
#             products = Product.objects.get(product_uuid=product_uuid)
#             print(products)
...
#             return Response({"error":"store_id not a vaild UUID."},status = status.HTTP_400_BAD_REQUEST)
#         try:
#             categories = Category.objects.filter(store_uuid=store_uuid)
...
#             return Response({"UUID":"UUID is not vaild."},status = status.HTTP_400_BAD_REQUEST)
#         try:
#             categories = Category.objects.get(category_uuid=category_uuid)
#         except Category.DoesNotExist:
